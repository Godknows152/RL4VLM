...
class MRFEB1(nn.Module):
    ...
    def forward(self, x):
        ...
        X_ = self.faix(x)
        N, D, _, _ = X_.shape
        w1 = torch.randn(self.dim_kernel, D).to(device)
        W = w1
        b = torch.FloatTensor(self.dim_kernel).uniform_(0, 2 * PI).to(device)
        B = b.repeat(N, 1)
        X_feature1 = torch.sqrt(torch.Tensor([2 / self.dim_kernel])).to(device)
        X_feature2 = torch.cos(torch.matmul(W, X_.T) + B.T).to(device)
